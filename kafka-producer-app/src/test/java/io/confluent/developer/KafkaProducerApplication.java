...
public class KafkaProducerApplicationTest {
    ...
    public void testProduce() throws IOException {
        ...
        records.forEach(producerApp::produce); // call  the produce method 
        ...
        final List<KeyValue<String, String>> expectedList = Arrays.asList(KeyValue.pair("foo", "bar"),
            ...
            KeyValue.pair(null,"great:weather")); // build the expected list of records the producer should receive

        final List<KeyValue<String, String>> actualList = mockProducer.history().stream().map(this::toKeyValue).collect(Collectors.toList()); // method to get records sent to producer so test can assert expected records match actual ones sent
